...
class User{
    ...
    */
    public function registration($email,$fname,$lname,$pass){
        ...
        $stmt = $pdo->prepare('INSERT INTO member (fname, lname, email, password, confirm_code) VALUES (?, ?, ?, ?, ?)');
        if($stmt->execute([$fname,$lname,$email,$pass,$confCode])){
            if($this->sendConfirmationEmail($email)){
                return true;
            }else{
                $this->msg = 'confirmation email sending has failed.2';
                return false; 
            }
        }else{
    ...
    */
    private function sendConfirmationEmail($email){
        $pdo = $this->pdo;
        $stmt = $pdo->prepare('SELECT email FROM member WHERE email = ? limit 1');
        $stmt->execute([$email]);
        $code = $stmt->fetch();
